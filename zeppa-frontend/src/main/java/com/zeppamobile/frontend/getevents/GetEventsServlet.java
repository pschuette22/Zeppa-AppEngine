...
public class GetEventsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String token = (String) req.getSession().getAttribute(UniversalConstants.PARAM_ID_TOKEN);
		...
		try {
			...
			resp.getWriter().println(String.format(format, "Event Title", "Location", "Start Time (ms)", "End Time (ms)") + "\n");
			// Get the objects array from the JSON response
			JSONObject obj = (JSONObject) JSONValue.parse(builder.toString());
			JSONArray objArr = (JSONArray) obj.get("objects");
			...
			for(int i=0; i < objArr.size(); i++) {
				...
				resp.getWriter().println(String.format(format, title, location, String.valueOf(start), String.valueOf(end)));
			}
