...
public class QueryHelper {
		...
		for (int i=0; i<NUM_TRIALS; i++) {
			for (POS posSource: POS.getAllPOS()) {
				for (POS posTarget: POS.getAllPOS()) {
					for (PointerType pt: PointerType.getAllPointerTypesForPOS(posSource)) {
						Synset tuple[] = getSynsets(pt, posSource, posTarget);
						
						if (tuple != null) { //Execute google search
							String source = tuple[0].getWords().get(0).getLemma();
							String target = tuple[1].getWords().get(0).getLemma();
							
							//If the words are the same get the next word from the target synset
							if (source.equalsIgnoreCase(target) && tuple[1].getWords().size() > 1) {
								target = tuple[1].getWords().get(1).getLemma();
							}
						...
						pw.println(pt.getLabel() + "\t" + posSource.getLabel() + "\t" 
								+ posTarget.getLabel() + "\t" + resultCount);
					}
				}
			}
			
			//Write results for each cycle to file in case the program crashes or gets killed
			pw.flush();
	 ...
	 * 
	 * @param pt - some PointerType
	 * @param source - S1.PartOfSpeech
	 * @param target - S2.PartOfSpeech
	 * @return a synset tuple (S1, S2) or null on timeout (1000ms)
	 */
	private static Synset[] getSynsets(PointerType pt, POS posSource, POS posTarget) {
		Synset source, target;
		List<PointerTarget> targets;
		IndexWord base;
		...
		try {
			...
			//Reenter loop if we have an empty list or we have not yet found a match for target POS
	 ...
	 * 
	 * @param set - a set of POS to choose from
