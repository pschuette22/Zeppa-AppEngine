...
public class SemSigProcess {
	...
	public static SemSigProcess getInstance() {
		try {
			if (instance == null)
				instance = new SemSigProcess();
			return instance;
		} catch (Exception e) {
			throw new RuntimeException("Could not init SemSigProcess: "
					+ e.getMessage());
		}
	}
	...
	private SemSigProcess() {
		...
		if (wordBasePathMap == null) {
			...
			String path = ADWConfiguration.getInstance().getWordPPVPath(
					LKB.WordNetGloss);
	...
	/**
	 * Assumes that the SemSigs are already sorted and normalized Checks the
	 * useLocalSignatures flag to download local or from cloud
	 * 
	 * @param path
	 * @param size
	 * @param warnings
	 ...
	 */
	public SemSig getCustomSemSigFromCompressed(String path, int size,
			boolean warnings, LKB normalizationLKB) {
		...
		try {
			...
			if (!br.ready()) {
				log.info("Buffered reader init failed, useCloudSignatures="+ADWConfiguration.getInstance().getSignaturesSource());
			}
			...
			log.error("[ERROR: Failed to get semantic signature resource, useCloudSignatures="
					+ ADWConfiguration.getInstance().getSignaturesSource()
					+ "]");
			e.printStackTrace();
		}
	...
	public List<SemSig> getWordSemSigs(String word, POS tag, LKB lkb, int size,
			boolean warnings) {
	...
	public List<SemSig> getWordSemSigs(String word, POS tag, LKB lkb, int size,
		...
		List<String> wordOffsets = (tag == null) ? GeneralUtils
				.getWordOffsets(word) : GeneralUtils.getWordOffsets(word, tag);
		...
		if (vectors.size() == 0 || vectors.size() != wordOffsets.size()) {
			if (warnings)
				log.info("[Warning! no vector or incomplete vectors generated for "
						+ word + ":" + tag + "]");
			// System.exit(0);
		}
	...
	public List<SemSig> getAllSemSigsFromWordPosList(List<String> wordPosList,
			ItemType srcTextType, int alignmentVecSize) {
		...
		case SENSE_OFFSETS:
			for (String offset : wordPosList)
				allSemSigs.add(getSemSigFromOffset(offset, LKB.WordNetGloss,
						alignmentVecSize, true));

			break;

		case WORD_SENSE:
			for (String wordSense : wordPosList)
				allSemSigs.add(getSemSigFromOffset(wordSense, LKB.WordNetGloss,
						alignmentVecSize, true));
		...
		case SENSE_KEYS:
			for (String senseKeys : wordPosList)
				allSemSigs.add(getSemSigFromOffset(senseKeys, LKB.WordNetGloss,
						alignmentVecSize, true));

			break;

		// To avoid bias towards more frequent senses, signatures are
		// macro-averaged
		case SURFACE_TAGGED:
			for (String surfaceTagged : wordPosList) {
				List<SemSig> vectors = getWordPosSemSigs(surfaceTagged,
						LKB.WordNetGloss, alignmentVecSize, true);
		...
		case SURFACE:
			...
			for (String wordPos : wordPosList) {
				List<SemSig> vectors = getWordPosSemSigs(wordPos,
						LKB.WordNetGloss, alignmentVecSize, true);
				allSemSigs.add(SemSigUtils.averageSemSigs(vectors));
			}
	 ...
	 * @return
	 */
	public SemSig getAveragedWordsSemSig(List<String> wordsPOS, LKB lkb,
			int size, boolean warnings, LKB normalizationLKB) {
		List<SemSig> vectors = new ArrayList<SemSig>();
		...
		for (String wordPOS : wordsPOS) {
			...
			List<SemSig> thisWordVectors = getWordSemSigs(lemma, pos, lkb,
					size, warnings, normalizationLKB);
	...
	public SemSig getAveragedWordSemSig(String word, POS tag, LKB lkb,
			int size, boolean warnings, LKB normalizationLKB) {
		List<SemSig> vectors = getWordSemSigs(word, tag, lkb, size, warnings,
				normalizationLKB);

		return SemSigUtils.averageSemSigs(vectors);
	}

	public List<SemSig> getWordPosSemSigs(String taggedWord, LKB lkb, int size,
			boolean warnings) {
		...
		if (SemSigProcess.getInstance().wordVectorsExist(lkb)) {
			...
			vector.add(SemSigProcess.getInstance().getSemSigFromWord(word, tag,
					lkb, size));
	 ...
	 * @param offset
	 * @param top
	 *            vector size (if 0, take all)
	 * @param lkb
	 *            determines the vectors path
	 * @return
	 */
	public SemSig getSemSigFromOffset(String offset, LKB lkb, int size,
		...
		return getCustomSemSigFromCompressed(basePath + getSubdirectory(offset)
				+ offset + ".ppv", size, warnings);
	}
	...
	public SemSig getSemSigFromOffset(String offset, LKB lkb, int size,
		...
		if (!basePathMap.containsKey(lkb))
			basePathMap
					.put(lkb, ADWConfiguration.getInstance().getPPVPath(lkb));
	...
	public SemSig getSemSigFromWord(String word, String tag, LKB lkb, int size,
			LKB normalizationLKB) {
		if (!wordBasePathMap.containsKey(lkb))
			wordBasePathMap.put(lkb, ADWConfiguration.getInstance()
					.getWordPPVPath(lkb));

		String basePath = wordBasePathMap.get(lkb);
		String path = basePath + word.substring(0, 1) + "/" + word + "." + tag
				+ ".ppv";
	...
	public static String getSubdirectory(String offset) {
		// return
		// offset.substring(0,2)+"/"+offset.substring(2,4)+"/"+offset.substring(4,6)+"/"+offset.substring(6,8)+"/";
		// return
		// offset.substring(0,2)+"/"+offset.substring(2,4)+"/"+offset.substring(4,6)+"/";
		return offset.substring(0, 2) + "/" + offset.substring(2, 4) + "/";
	}

	public SemSig getSemSigFromIWord(IWord sense, LKB lkb, int size) {
		String offset = GeneralUtils.fixOffset(sense.getSynset().getOffset(),
				sense.getPOS());
	...
	public static void main(String args[]) {
		// SemSig v1 =
		// SemSigProcess.getInstance().getSemSigFromWordSense("gem.n.3",
		// LKB.WordNetGloss, 0);
		// SemSig v2 =
		// SemSigProcess.getInstance().getSemSigFromWordSense("jewel.n.2",
		// LKB.WordNetGloss, 0);

		// double sim = SemSigComparator.compare(v1, v2, new WeightedOverlap(),
		// 0, true, true);
		// System.out.println(sim);

		System.out.println(SemSigProcess.getInstance().getSemSigFromWord(
				"monkey", POS.NOUN, LKB.WordNetGloss, 0, null));
	}

	/**
	 * gets a word#pos and returns all the offsets associated with that word and
	 * part of speech
	 * 
	 * @param wordPos
	 * @return
	...
	public Set<String> getWordNetOffsets(String wordPos) {
		...
		if (wordPos.matches("[0-9]*-[nrva]")) {
			...
			List<IWord> senses = WordNetUtils.getInstance()
					.getSenses(word, pos);
			...
			for (IWord sense : senses) {
				offsets.add(GeneralUtils.fixOffset(sense.getSynset()
						.getOffset(), pos));
			}
		}
